...
def get_neighbour_cache(g, vertex_list):
    neighbour_cache = {}
    for v in vertex_list:
        temp = []
        for o in g.vertex(v).out_neighbours():
            temp.append(int(o))
        neighbour_cache[v] = temp 
    return neighbour_cache
...
def generate_and_save_path(g, start, neighbour_cache):
    current = start
    path_list = [start]
...
def generate_and_save_path_fast(g, start, neighbour_cache):
    current = start
    length = 0
...
def run_sequential_particles(g, start_vertex):
    ...
    # I think this is important because otherwise would return
    # a newly created np array each time
    neighbour_cache = get_neighbour_cache(g, g.get_vertices())

    list_of_particle_paths = []

    for i in range(g.num_vertices()):
        list_of_particle_paths.append(generate_and_save_path(g, start_vertex, neighbour_cache))
        ...
        for i in range(g.num_vertices()):
            list_of_particle_lengths.append(
                    generate_and_save_path_big(
                        i, n, g, start_vertex, neighbour_cache, filename))
    else:
        for i in range(g.num_vertices()):
            list_of_particle_lengths.append(
                    generate_and_save_path_fast(
                        g, start_vertex, neighbour_cache))

    reset_graph(g)
    t1 = time.time()
    print("slow time:", t1 - t0)
    return list_of_particle_lengths

# Run single path
def run_single_path(g):

    neighbour_cache = get_neighbour_cache(g, g.get_vertices())
    length = generate_and_save_path_fast(g, 0, neighbour_cache)
    return length
